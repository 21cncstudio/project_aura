...
Current development: local `main` in `aura-post-115-clean`, consolidated on
2026-09-17 at the user's request. The former `idf-6.1-migration` branch, based on
`78f4e9ba`, remains a historical checkpoint. Both newer main history commits are
retained. See [the consolidation report](MAIN_IDF61_UI_CLEANUP_20260917.md).
...
and `78f4e9ba` (dependency audit). Continue firmware changes on main.
...
The old firmware baseline remains at `codex/aura-main-before-idf61-20260917`
and in the exact retained device artifacts.
